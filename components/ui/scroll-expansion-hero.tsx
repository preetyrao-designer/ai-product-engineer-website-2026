...
export default function ScrollExpansionHero() {
  ...
  const width = useTransform(scrollYProgress, progress => `calc(min(300px, 78vw) * ${1 - progress} + 95vw * ${progress})`);
  const height = useTransform(scrollYProgress, progress => `calc(min(400px, 58svh) * ${1 - progress} + 85svh * ${progress})`);
  const left = useTransform(scrollYProgress, [0, 1], ['0vw', '-110vw']);
  const right = useTransform(scrollYProgress, [0, 1], ['0vw', '110vw']);
  const backgroundOpacity = useTransform(scrollYProgress, [0, 0.94], [1, 0]);
  const introOpacity = useTransform(scrollYProgress, [0.38, 0.81], [0, 1]);
  const openingOpacity = useTransform(scrollYProgress, [0, 0.25], [1, 0]);
  ...
  return <section ref={section} className={`sri-expansion${reduced ? ' sri-expansion-reduced' : ''}`} aria-labelledby="sri-experience-title">
    <div className="sri-expansion-stage">
      ...
      <div className="sri-expansion-copy" style={reduced ? { display: 'none' } : undefined}>
        {showOpening && <motion.p className="sri-expansion-opening" style={{ opacity: openingOpacity }}>Have an immersive experience<br />in Sri Lanka with our</motion.p>}
