...
export interface TimelineEntry { title: string; titleContent?: ReactNode; content: ReactNode }
...
export function Timeline({ data }: { data: TimelineEntry[] }) {
  ...
  return <ol ref={ref} className="admission-timeline">
    ...
    {data.map((item, i) => <li key={item.title} className="admission-step">
      ...
      <article className="admission-step-card"><h3>{item.titleContent ?? item.title}</h3>{item.content}</article>
    </li>)}
  </ol>;
}
